...
async def bull_thesis_node(state: TradingAnalysisState):
    logger.info("🐂 [5A] Bull Analyst (Round 1): Drafting independent thesis...")
    ...
    prompt = f"""You are a Bull Analyst for {ticker}.
Your task is to Identify exactly WHY the asset will rise.
...
Your task is to review the following Phase 1 Data Reports and formulate a strong, quantitative Bullish Thesis for the next 1-4 weeks.
You must NOT see or reference the Bear's arguments. Focus ONLY on why the data supports an upside move.
For safe-haven assets like Gold (GC=F), focus on macroeconomic tailwinds (inflation, falling rates) and retail fear.
...
Write a concentrated, highly specific Bullish Thesis. Avoid conversational filler. State your exact rationale."""

    response = await llm.ainvoke([SystemMessage(content=prompt)])

    # Initialize dict if None
    debate_state = state.get("investment_debate_state") or {}
    debate_state["bull_thesis"] = response.content
